...
public class Lab1 {
	...
	public static int removeLastTwoDigits(int n) {
		String tempVar = Integer.toString(n); // converts int n into a String
												// for easier manipulation
		int length = tempVar.length(); // finds how many digits the variable is

		return Integer.parseInt(tempVar.substring(0, length - 2));
	}
	...
	public static int lastTwoDigits(int n) {
		String tempVar = Integer.toString(n); // converts int n into a String
												// for easier manipulation
		int length = tempVar.length(); // finds how many digits the variable is
		if (String.valueOf(tempVar.charAt(length - 2)).equals("0")) {
			return Integer.parseInt(tempVar.substring(length - 1, length));
		} else {
			return Integer.parseInt(tempVar.substring(length - 2, length));
		}
	}
	...
	public static int yourAge(int address, int birthYear, int birthdays) {
		int answer = ((address * 2) + 42) * 50;
		answer = (answer - birthYear) - 50;
		answer = (answer + birthdays) - 34;

		int length = Integer.toString(answer).length();

		answer = Integer.parseInt(Integer.toString(answer).substring(length - 2));
	...
	public static double windChill(double airTemp, double windSpeed) {
		...
		if (airTemp > 0.0) {
			throw new Lab1Exception("The value of " + airTemp + " must be equal to or less than 0!");
		} else if (windSpeed < 5.0) {
			throw new Lab1Exception("The value of " + windSpeed + " must be equal to or greater than 5!");
		} else {
			double answer;
			answer = 13.12 + (0.6215 * airTemp) + (0.3965 * airTemp - 11.37) * Math.pow(windSpeed, 0.16);
			return answer;
		}
	...
	/**
	 * Determine if the point <code>(x, y)</code> is 
	 * inside the unit square. The
	 * unit square is the square whose sides 
	 * have length 1, whose bottom left
	 * corner has coordinates (0, 0), and 
	 * whose top right corner has coordinates
	 * (1, 1). A point on the perimeter of the
	 *  unit square is considered to be
	 * inside the square.
	 * 
	 * @param x
	 *            the x-coordinate of the point
	...
	public static int yourAgeChecked(int address, int birthYear, int birthdays) {
		...
		if ((address < MIN_ADDRESS) || (address > MAX_ADDRESS)) {
			throw new IllegalAddressException("The value of " + address + " is either too large or too small");
		}

		else if ((birthYear < MIN_BIRTH_YEAR) || (birthYear > MAX_BIRTH_YEAR)) {
			throw new IllegalBirthYearException("The value of " + birthYear + " is either too large or too small");
		}

		else if ((birthdays < 0) || (birthdays > 1)) {
			throw new IllegalBirthdaysException("The value of " + birthdays + " must be 0 or 1");
	...
	public static char middleChar(String s) {
		...
		if (s.length() == 0) {
			...
			if (s.length() % 2 == 0) {
				midChar = s.charAt((s.length() / 2));
			} else {
				midChar = s.charAt((s.length() - 1) / 2);
			}
		}
		return midChar;
	}
